...
def mean_squared_error(y_true, y_predicted):
    """ Calculating the loss or cost given the observed and predicted """
    return np.sum((y_true - y_predicted)**2) / len(y_true)

def gradient_descent(x, y, iterations=1000, learning_rate=0.0001, stopping_threshold=1e-6):
    """ Gradient Descent Function
        iterations, learning_rate, and stopping_threshold are hyperparameters
        that can be tuned by the user
        (hyperparameters are variables that are pre-configured before training the ML model
     """
    ...
    for i in range(iterations):
        ...
        # if the change in cost is <= the stopping threshold we stop the gradient descent
